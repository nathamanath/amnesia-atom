...
const getExtention = (path) => {
  ...
  if(file && file.path.match(/\.([0-9a-z]+$)/i)) {
    return file.path.match(/\.([0-9a-z]+$)/i)[1];
  } else {
    return getConfiguredFileFormat();
  }
}
...
export default {
  shareBlock: function() {
    ...
    shareCode(blocks.join(SCOPE_DELIMITER), getExtention(), SHARE_TYPES.BLOCK);
  },
  ...
  shareSelection: function() {
    ...
    if(texts.length > 0) {
      shareCode(texts.join(SCOPE_DELIMITER), getExtention(), SHARE_TYPES.SELECTION);
    } else {
  ...
  shareFile: function() {
    let code = atom.workspace.getActivePaneItem().buffer.cachedText;

    shareCode(code, getExtention(), SHARE_TYPES.FILE);
  },
  ...
  shareLine: function() {
    ...
    shareCode(rows.join(SCOPE_DELIMITER), getExtention(), SHARE_TYPES.LINE);
  }
}
