...
export default {
  ...
  shareBlock: function() {
    ...
    let blocks = editor.cursors.map((cursor) => {
      ...
      return normalizeSpacing(editor.getTextInRange(range))
    })

    shareCode(blocks.join(SCOPE_DELIMITER), getExtention(path), SHARE_TYPES.BLOCK)
  },
  ...
  shareSelection: function() {
    ...
    let texts = selections.map((selection) => {
      return normalizeSpacing(selection.getText())
    }).filter((t) => { return t.length })

    if(texts.length > 0) {
      shareCode(texts.join(SCOPE_DELIMITER), getExtention(path), SHARE_TYPES.SELECTION)
    } else {
  ...
  shareFile: function() {
    ...
    let path   = file ? file.path : ''
    let editor = atom.workspace.getActivePaneItem()
    let code   = normalizeSpacing(editor.buffer.getLines().join('\n'))
  ...
  shareLine: function() {
    ...
    let rows = editor.cursors.map((cursor) => {
      ...
      return normalizeSpacing(editor.buffer.getLines()[row])
    })
