...
export default {
  shareBlock: function() {
    ...
    shareCode(block, getExtention(getFile().path), SHARE_TYPES.BLOCK);
  },
  ...
  shareSelection: function() {
    ...
    if(selection.length > 0) {
      shareCode(selection, getExtention(getFile().path), SHARE_TYPES.SELECTION);
    } else {
  ...
  shareLine: function() {
    ...
    shareCode(editor.buffer.lines[row], getExtention(path), SHARE_TYPES.LINE);
  }
}
